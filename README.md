...
**Who qualifies:** the buyer is matched by role and goal, not by every attribute the website lists. A different platform, audience or company size, or niche (a TikTok creator for an offer described for Instagram creators, a follower count not given) qualifies with a caveat and a lower fit score, unless the profile's exclusions or the user's focus rule it out; a different role (seller, adviser, the buyer's customer, someone without the audience or business the offer works on) is rejected. Leads that qualify but lose a run's top-five cut stay in `research_candidates` as `qualified_not_selected` and compete on score with the next run's finds, so they are shown on a later day instead of being lost.
