...
import { formatDate, formatDateTime, localDateKey, Panel } from './ui'
...
export function Results({ view, privateUrl, actions }: { view: RadarView; privateUrl: string; actions: Actions }) {
  ...
  else if (view.leads.length < 3) heading = `We found ${view.leads.length} strong ${view.leads.length === 1 ? 'match' : 'matches'} so far`
  else heading = `${total} ${total === 1 ? 'opportunity' : 'opportunities'} for ${name}`
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-4">
        <Subheading className="max-w-4xl">
          {heading}
          {!noLeadsYet && view.leads.length < 3 && <span className="text-mist-600 dark:text-mist-400"> for {name}</span>}
        </Subheading>
        {view.focus && (
          <p className="text-base/7 text-pretty text-mist-700 dark:text-mist-400">
            <span className="text-mist-950 dark:text-white">Searching for:</span> {view.focus.services} ·{' '}
            {view.focus.customerType} · {view.focus.market}{' '}
            {!view.expired && !editingFocus && (
              <button
                type="button"
                onClick={() => setEditingFocus(true)}
                className="text-sm/7 font-medium whitespace-nowrap text-mist-950 underline decoration-mist-950/20 underline-offset-4 hover:decoration-mist-950 dark:text-white dark:decoration-white/20"
              >
                Adjust focus
              </button>
            )}
        ...
        {view.sampleData && (
          <p className="rounded-lg bg-amber-50 px-4 py-2 text-sm/6 text-amber-900 inset-ring-1 inset-ring-amber-600/20 dark:bg-amber-950/40 dark:text-amber-200">
      ...
      <div className="grid grid-cols-1 gap-8 *:min-w-0 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
        <aside className="lg:sticky lg:top-6 lg:order-last" aria-label="Research status">
        ...
        <div className="flex flex-col gap-10">
          {noLeadsYet && (
            <Panel className="flex flex-col gap-4">
              ...
              <Text className="text-sm/6">If this isn’t the right page for your services, try a more specific URL:</Text>
              <div className="[&_form]:mt-0 [&_form]:max-w-none">
                <WebsiteForm />
              </div>
            </Panel>
          )}
          ...
          {[...groups.entries()].map(([key, leads]) => (
            <section key={key} aria-labelledby={`group-${key}`} className="flex flex-col gap-3">
              <h2 id={`group-${key}`} className="text-sm/7 font-semibold text-mist-700 dark:text-mist-400">
                {groupLabel(key, view)} · {leads.length} {leads.length === 1 ? 'lead' : 'leads'}
              </h2>
